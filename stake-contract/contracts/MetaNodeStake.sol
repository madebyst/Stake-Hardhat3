    ...
    AccessControlUpgradeable
{
    ...
    */
    ...
    function updatePool(uint256 _pid) public checkPid(_pid) {
        ...
        (bool success1, uint256 totalMetaNode) = getMultiplier(pool_.lastRewardBlock, block.number)
        ...
        if (stSupply > 0) {
            ...
            (bool success3, uint256 accMetaNodePerST) = pool_.accMetaNodePerST.tryAdd(
                totalMetaNode_
            );
    ...
    function _deposit(uint256 _pid, uint256 _amount) internal {
        ...
        if (user_.stAmount > 0) {
            ...
            if (pendingMetaNode_ > 0) {
                (bool success3, uint256 _pendingMetaNode) = user_.pendingMetaNode.tryAdd(
                    pendingMetaNode_
                );
